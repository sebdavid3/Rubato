...
const DonationMethodCard: React.FC<{
  ...
  onClick?: () => void;
  showDetails?: boolean;
}> = ({ title, details, icon, actionText, actionUrl, isExternal = false, onClick, showDetails = false }) => {
  ...
  return (
    <div className="bg-bgDarkSection rounded-xl p-6 hover:shadow-xl transition-all duration-300 border border-bgDark hover:border-accent/30 h-full flex flex-col">
      ...
      {!showDetails && (
        <div className="flex-grow mb-4">
          <p className="text-textSecondary font-montserrat text-sm">
            {title === "Transferencia Bancaria" ? "Haz clic para ver los datos bancarios" : "Donación rápida y segura"}
          </p>
        </div>
      )}
      ...
      <div className="mt-auto">
        ...
        ) : title === "Transferencia Bancaria" && showDetails ? (
          <button 
  ...
  const donationMethods = [
    {
      ...
      actionText: showBankDetails ? "Copiar datos bancarios" : "Ver datos bancarios",
