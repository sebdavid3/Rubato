...
const DonationMethodCard: React.FC<{
  ...
  actionUrl?: string;
  isExternal?: boolean;
}> = ({ title, details, icon, actionText, actionUrl, isExternal = false }) => {
  return (
    <div className="bg-bgDarkSection rounded-xl p-8 hover:shadow-xl transition-all duration-300 border border-bgDark hover:border-accent/30">
      <div className="flex items-center mb-6">
        <div className="w-16 h-16 bg-accent rounded-lg flex items-center justify-center mr-4">
          {icon}
        </div>
        <h3 className="text-xl font-bold text-textLight font-cinzel uppercase">
          {title}
        </h3>
      </div>
        ...
        <div className="space-y-3 mb-6">
          {details.map((detail, index) => (
            <div key={index} className="flex items-start gap-3">
              ...
              <p className="text-textSecondary font-montserrat">{detail}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {actionUrl && (
        <a 
          ...
          className="inline-flex items-center gap-2 bg-accent text-textLight px-6 py-3 rounded-lg font-medium hover:bg-primary transition-colors font-montserrat"
        >
          {actionText}
          {isExternal && (
  ...
  const donationMethods = [
    {
      ...
      actionText: "Copiar datos bancarios",
      actionUrl: "#"
    },
    {
      title: "Mercado Pago",
